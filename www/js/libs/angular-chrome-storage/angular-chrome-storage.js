...
angular.module("chromeStorage", [])
    .factory('chromeStorage', function ($q) {
        ...
        var area = null;
        try {
            area = chrome.storage.sync; // change this to chrome.storage.sync for sync capabilities chrome.storage.local
        } catch (err) {
        ...
        return {
            ...
            get: function (key) {
                ...
                getValue(key).then(function (keyValue) {
                    value = keyValue;
                    set = true;
                });
