...
export class WeatherProvider {
  ...
  constructor (public http: HttpClient, private store: Store<SettingsState>) {
    ...
    public getForcast(lat: number, lon: number, units: string, lang: string): Observable<Weather> {
        ...
        return this.settingsState$.pipe(take(1)).pipe(
          switchMap((settings) => 
            this.http.get<Weather>(`https://api.openweathermap.org/data/3.0/onecall?lat=${lat}&lon=${lon}&units=${units}&lang=${lang}&appid=${settings?.appSettings?.OpenWeatherApiKey}`)
          ));
    };
}
