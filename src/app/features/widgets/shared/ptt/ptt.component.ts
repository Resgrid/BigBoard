...
export class PTTWidgetComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    private widgetStore: Store<WidgetsState>,
    ...
    this.widgetSettingsState$ = this.widgetStore.select(
      selectPTTWidgetSettingsState,
    );
  }
          ...
          if (state && !state.currentActiveVoipChannel) {
            this.widgetSettingsState$.pipe(take(1)).subscribe((settings) => {
              if (settings && settings.Channel) {
                const channel = _.find(state.channels, {
                  Name: settings.Channel,
                });

                if (channel) {
                  this.voiceStore.dispatch(
                    new VoiceActions.SetActiveChannel(channel),
                  );
                }
              }
            });
