...
export const WidgetContainer: React.FC<WidgetContainerProps> = ({ title, onRemove, children, isEditMode = false, testID, width, height }) => {
  ...
  return (
    <Box className={`overflow-hidden rounded-lg ${isDark ? 'border border-gray-700 bg-gray-800' : 'border border-gray-200 bg-white'}`} style={{ width, height }} data-testid={testID}>
      {/* Header */}
      <HStack
      ...
      >
        ...
        {isEditMode && onRemove && (
          <Pressable onPress={onRemove} className="p-1" data-testid={testID ? `${testID}-remove-button` : undefined}>
