...
export class UtilsProvider {
	...
	public formatDateString(date: Date) {
		...
		*/
		...
		let time = this.padLeadingZero(month) + "/" + this.padLeadingZero(day) + "/" + year + " " + this.padLeadingZero(hour) + ':' + this.padLeadingZero(minute) + ':' + this.padLeadingZero(second) + " " + timeZone;
	...
	private createDateUTCOffset(date) {
		...
		var hours = this.padLeadingZero(Math.floor(offset / 60));
		var minutes = this.padLeadingZero(offset % 60);
	...
	public getTimeAgoUtc(time) {
		...
		time = Number((new Date(time).getTime() + new Date(time).getTimezoneOffset() * 60 * 1000));
		...
		var seconds = (Number((new Date(currentDate).getTime() + new Date(currentDate).getTimezoneOffset() * 60 * 1000)) - time) / 1000,
			token = 'ago',
			list_choice = 1;
